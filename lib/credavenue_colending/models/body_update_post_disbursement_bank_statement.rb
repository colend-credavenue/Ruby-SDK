...
module Colending
  ...
  class BodyUpdatePostDisbursementBankStatement
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Colending::BodyUpdatePostDisbursementBankStatement` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Colending::BodyUpdatePostDisbursementBankStatement`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'perfios_link')
        self.perfios_link = attributes[:'perfios_link']
      end

      if attributes.key?(:'finbit_link')
        self.finbit_link = attributes[:'finbit_link']
      end
    ...
    def hash
      [bank_statement_savings_account_12_months_link, bank_statement_current_account_12_months_link, no_of_inward_chq_returns_savings_account, no_of_outward_chq_returns_savings_account, no_of_inward_chq_returns_current_account, no_of_outward_chq_returns_current_account, bank_account_type_for_assessment, perfios_link, finbit_link, abb, abd].hash
    end
