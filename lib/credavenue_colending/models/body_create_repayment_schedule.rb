...
module Colending
  class BodyCreateRepaymentSchedule
    # Refer table RepaymentSchedules for attributes
    attr_accessor :repayment_schedules
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Colending::BodyCreateRepaymentSchedule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Colending::BodyCreateRepaymentSchedule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'repayment_schedules')
        if (value = attributes[:'repayment_schedules']).is_a?(Array)
          self.repayment_schedules = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          repayment_schedules == o.repayment_schedules
    end
