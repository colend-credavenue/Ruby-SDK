  ...
  class BodyCreateLoanLinkedLoanInfos
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Colending::BodyCreateLoanLinkedLoanInfos` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Colending::BodyCreateLoanLinkedLoanInfos`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'linked_loan_id')
        self.linked_loan_id = attributes[:'linked_loan_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          linked_loan_id == o.linked_loan_id &&
    ...
    def hash
      [linked_loan_id, foreclosure, principal, interest, other_charges, overdue_charges, penalty_charges, fee_charges, bounce_charges, amount].hash
    end
