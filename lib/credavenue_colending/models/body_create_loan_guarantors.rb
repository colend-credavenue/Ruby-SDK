...
module Colending
  ...
  class BodyCreateLoanGuarantors
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Colending::BodyCreateLoanGuarantors` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Colending::BodyCreateLoanGuarantors`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [relationship_with_applicant, first_name, middle_name, last_name, date_of_birth, age, gender, current_address, current_district, current_city, current_city_tier, current_state, current_pincode, years_of_stay_in_current_address, residence_type_current_address, permanent_address, permanent_district, permanent_city, permanent_city_tier, permanent_state, permanent_pincode, years_of_stay_in_permanent_address, residence_type_permanent_address, mobile_number, email, pan_number, pan_number_link, pan_number_link_password, aadhar_number, aadhar_number_link, aadhar_number_link_password, voter_id_number, voting_id_number_link, voting_id_number_link_password, driving_license_number, driving_licence_number_link, driving_licence_number_link_password, passport_number, passport_number_link, passport_number_link_password, video_image_link, video_image_link_password, video_kyc_detail, other_kyc_number, other_kyc_number_link, other_kyc_number_link_password, rent_agreement_link, rent_agreement_link_password, utility_bills_link, utility_bills_link_password, photo_link, photo_link_password, marital_status, religion, nationality, bureau_score, bureau_report_link, bureau_report_link_password, partner_score].hash
    end
