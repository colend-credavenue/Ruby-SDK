...
module Colending
  # Refer table ConsumerDurable for attributes
  class BodyCreateLoanConsumerDurable
    # Product Category
    attr_accessor :product_category

    # Down Payment Amount for product purchased
    attr_accessor :down_payment_amount
    ...
    # Year when the product was manufactured
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Colending::BodyCreateLoanConsumerDurable` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Colending::BodyCreateLoanConsumerDurable`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'down_payment_amount')
        self.down_payment_amount = attributes[:'down_payment_amount']
      end
      ...
      if attributes.key?(:'model_year')
        self.model_year = attributes[:'model_year']
      end
    end
    ...
    def hash
      [product_category, down_payment_amount, product_price, retailer_name, manufacturer_name, model_name, model_year].hash
    end
